...
```{r setup}
librarian::shelf(tidyverse, here, urbnthemes, gt, tidycensus, mapview)
...
data %>% 
  ...
  summarise(allocated = sum(allocation, na.rm = TRUE), .by = "building_block") %>% 
  # mutate(allocated = scales::label_dollar(scale = 1/1e6, suffix = "M")(allocated),
  #        total = "") %>% 
  ungroup() %>% 
  ggplot(aes(x = "", y = allocated, fill = building_block, label = scales::label_dollar(accuracy = 1, scale = 1/1e6, suffix = "M")(allocated))) +
  geom_col(position = position_stack(reverse = TRUE), width = 0.4) +
  ...
  scale_y_continuous(labels = scales::label_dollar(scale = 1/1e6, suffix = "M"),
                     breaks = seq(0, 300 * 1e6, 100*1e6),
                     limits = c(0, 300*1e6)) +
  scale_fill_manual(values = racf_palette[c(3, 5)])
...
data %>% 
  ...
  summarise(allocated = sum(allocation, na.rm = TRUE), .by = "building_block") %>% 
  ungroup() %>% 
  ggplot(aes(x = "", y = allocated, fill = building_block, label = scales::label_dollar(scale = 1/1e6, suffix = "M", accuracy = 1)(allocated))) +
  geom_col(position = position_stack(reverse = TRUE), width = 0.4) +
  geom_text(size = 3, position = position_stack(vjust = 0.5, reverse = TRUE)) +
  coord_flip() +
  labs(x = NULL, y = NULL) +
  scale_y_continuous(labels = scales::label_dollar(scale = 1/1e6, suffix = "M", accuracy = 1),
                     breaks = seq(0, 300 * 1e6, 100*1e6),
                     limits = c(0, 300*1e6)) +
    scale_fill_manual(values = racf_palette[c(3, 5)])
...
```{r}
data %>% 
  filter(geography == "Monroe County") %>% 
  mutate(building_block = if_else(is.na(building_blocks), "Allocated to other programs", "Allocated to a building block")) %>% 
  summarise(allocated = sum(allocation, na.rm = TRUE), .by = "building_block") %>% 
  ungroup() %>% 
  ggplot(aes(x = "", y = allocated, fill = building_block, label = scales::label_dollar(scale = 1/1e6, suffix = "M", accuracy = 1)(allocated))) +
  geom_col(position = position_stack(reverse = TRUE), width = 0.4) +
  geom_text(size = 3, position = position_stack(vjust = 0.8, reverse = TRUE)) +
  coord_flip() +
  labs(x = NULL, y = NULL) +
  scale_y_continuous(labels = scales::label_dollar(scale = 1/1e6, suffix = "M", accuracy = 1),
                     breaks = seq(0, 300 * 1e6, 100*1e6),
...
project_locations <-
  ...
  left_join(spending %>% select(street, arpa_funding), by = c("f_address" = "street"))
# Create the map
...
m <- m + mapview(project_locations, zcol = 'type', cex = "arpa_funding", col.regions = colorRampPalette(racf_palette)) # Use blue to red color scale for project types
...
```{r}
  ...
  mapview(list(select(race, pct_black), project_locations),
          zcol = list("pct_black", "type"),
          cex = list("", "arpa_funding"),
          layer.name = list("%Black", "Project Locations"),
          col.regions = list(colorRampPalette(palette_urbn_cyan), colorRampPalette(racf_palette)))
...
```{r}
m <-  mapview(list(select(race, pct_hispanic), project_locations),
          zcol = list("pct_hispanic", "type"),
          layer.name = list("%Hispanic", "Project Locations"),
          cex = list("", "arpa_funding"),
          col.regions = list(colorRampPalette(palette_urbn_green), colorRampPalette(racf_palette)))
