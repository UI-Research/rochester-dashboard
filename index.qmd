...
# Rochester Region Federal Funds Dashboard Mockup
...
```{r setup}
librarian::shelf(tidyverse, here, urbnthemes, gt, tablexlsx)
...
data %>% 
  ...
  select(program, program_description, geography, agency, topic, building_blocks, racf_categories, flexible_fund, allocated = allocation, spent, current_as_of) %>% 
  ...
  cols_width(
    Program ~ px(150),
    `Program description` ~ px(250),
    Geography ~ px(150),
    Topic ~ px(150)
  ) %>% 
  fmt_number(c(Allocated, Spent), decimals = -1) %>% 
  data_color(columns = c(Allocated, Spent), method = "numeric", palette = "viridis") 
```
