...
equitable growth. @fig-pct-allocated-inclusive-recovery below shows the total amount of funding for
...
```{r}
#| label: fig-pct-allocated-inclusive-recovery
#| fig-cap: !expr source
...
barriers to employment. @fig-building-blocks-allocation below shows how that funding is divided among
the five building blocks and includes only the funds allocated to programs that
align with the building blocks (the 65%).

```{r}
#| label: fig-building-blocks-allocation
#| fig-cap: !expr source
...
@fig-policy-areas-allocation shows how funding has been allocated across policy categories
...
```{r}
#| label: fig-policy-areas-expenditure
#| fig-cap: !expr source
...
data %>%
  ...
  geom_col() +
  geom_text(aes(label = scales::label_number(scale = 1 / 1e6, suffix = "", prefix = "$", accuracy = 1)(spent)),
    position = position_stack(vjust = 0.5),
    # hjust = -0.1,
    color = "white",
    size = 4, fontface = "bold"
