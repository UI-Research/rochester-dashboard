...
redlining <- 
  sf::read_sf("data/data-raw/fullDownload.geojson.txt") %>%
  filter(city == 'Rochester', state == 'NY') 
  

project_locations <-
  read_csv("data/addresses-geocoded.csv") %>% 
  janitor::clean_names() %>% 
  filter(longitude != 0) %>% 
...
m <- mapview(redlining, zcol = 'holc_grade', map.types = "OpenStreetMap")
m <- m + mapview(project_locations, zcol = 'type', col.regions = colorRampPalette(c("cyan", "red", "purple", "orange"))) # Use blue to red color scale for project types
...
The code used to create this dashboard can be found on
